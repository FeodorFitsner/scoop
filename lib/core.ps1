...
if ($MyInvocation.mycommand.Definition) { $projectrootpath = (Split-Path -Parent $MyInvocation.MyCommand.Definition) }
...
function ConvertFrom-JsonPoSH2 {
    ...
    BEGIN {
        if (-not (Get-Module 'Newtonsoft.Json')) {
            $modulePath = $(resolve-path $(rootrelpath 'vendor\Newtonsoft.Json\lib\net20\Newtonsoft.Json.dll'))
            $modulePath
