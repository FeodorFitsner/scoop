...
if (-not (test-path alias:\ConvertFrom-Json20)) { new-alias ConvertFrom-Json20 ConvertFrom-Json }
if (-not (test-path alias:\ConvertTo-Json20)) { new-alias ConvertTo-Json20 ConvertTo-Json }
...
function ConvertFrom-JsonPoSH2 {
    ...
    BEGIN {
        if (-not (Get-Module 'Newtonsoft.Json')) {
            ...
            $filename = [System.IO.Path]::ChangeExtension([System.IO.Path]::GetTempFileName(),'.dll')
            copy-item -force $(resolve-path $(rootrelpath 'tools\Newtonsoft.Json\lib\net20\Newtonsoft.Json.dll')) $filename
